...
from settings import MAX_PAGES, PDF_DIR, PDF_NAME, DOCX_DIR, DOCX_NAME, REPLACE_OFF_FORMAT, REPLACE_ON_FORMAT, SLEEP_TIME
...
class OCRpdf(OCRClient):
    ...
    def __init__(self, pdf_patch, language_codes, TOKEN, **kwargs):
        ...
        self.pdf_patch = os.path.join("/".join(os.getcwd().split("/")[:-2]), pdf_patch) # pdf_patch like raw_pdf/pdf_document_0.pdf
        self.tacks_ids = []
    ...
    def create_ocr_pdf_tasks(self):
        ...
        pdf_file_names.sort(key=lambda x: int(x.split("_")[-1].split(".")[0])) # sort by pdf id
        ...
        for pdf_file_name in pdf_file_names:
            ...
            response_json = self.post_process_request(json_body=json_body, handle="pdf_async")
    ...
    def get_ocr_pdf_tasks_result(self):
        ...
        while idx < len(self.tacks_ids):
            try:
                text_response = self.get_process_request(handle="get_pdf_async", id=self.tacks_ids[idx])
            except:
                text_response = None
            ...
            idx += 1
            json_result = json.loads("[" + text_response.replace(REPLACE_OFF_FORMAT, REPLACE_ON_FORMAT) + "]")
...
def parse_args():
    ...
    parser.add_argument('--language_code',  help='list input with "," delimited', type=lambda s: [item for item in s.split(',')], default=['ru', 'en'])
...
def main(args):
    ...
    ocr_pdf = OCRpdf(pdf_patch=args.pdf_path, language_codes=args.language_code, TOKEN=TOKEN)
